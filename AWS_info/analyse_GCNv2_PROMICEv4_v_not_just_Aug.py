...
# if site=='JAR':
#     fn='/Users/jason/Dropbox/AWS/GCNv2/xmitted_data.old_see\ upper\ folder/AWS_300534060520350.txt'
# # else:
...
if ((site=='SWC')or(site=='NUK_U')or(site=='QAS_U')or(site=='JAR')):
    sensor_levels=['L']
else:
    sensor_levels=['L','U']
...
var=(df['Rain_amount_L']+df['Rain_amount_U'])/2
...
for i,sensor_level in enumerate(sensor_levels):
    ...
    ax[cc].plot(xt,y,
            ...
            # label='air T '+str(i+1),
            linewidth=th*2)#,label='air T, '+sensor_level)   
    ax[cc].fill_between(xt, y, where=(y > 0), color='orange', alpha=.8)
...
ax.plot(x1)
# plt.plot(x2)
ax.set_title('Swiss Camp surface height record')
...
if ((site!='SWC')and(site!='NUK_U')):
    ...
    SR50=x1
    xt=df.time[t0:t1]
    ax[cc].fill_between(xt,SR50[t0:t1], where=(SR50[t0:t1] > 0), color='k', alpha=.1,label='snow')
...
ax[cc].plot(SR50[t0:t1],
            ...
            linewidth=th*2, color='grey',label='surface height')  
