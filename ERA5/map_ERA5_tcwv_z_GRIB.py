...
if os.getlogin() == 'jason':
    ...
    heavy_data_path='/Users/jason/0_dat/ERA5/'
...
fn=heavy_data_path+'ERA5_6hourly_10-31_Aug_2021_z.grib'
...
if do_turb:
    fn=heavy_data_path+'ERA5_6hourly_10-31_Aug_2021_turb_fluxes.grib'
    ds_turb = cfgrib.open_dataset(fn)
    print(ds_turb)
else:
    fn=heavy_data_path+'ERA5_6hourly_10-31_Aug_2021_tcwv.grib'
...
# for i in range(ihour,ihour+1):
    ...
    if msg!='x2021-08-13-12':
        ...
        if do_turb:
            # tcwv = ds_turb.variables['slhf'][i,1,:,:]/1e4
            # namx='LHF'
            tcwv = ds_turb.variables['sshf'][i,1,:,:]/1e4
            namx='SHF'
            prefix=namx+", "
            lo=-100; hi=100
            dx=(hi-lo)/20
    ...
    # https://stackoverflow.com/questions/33637693/how-to-use-streamplot-function-when-1d-data-of-x-coordinate-y-coordinate-x-vel/33640165#33640165
        ...
        if ly == 'p':
            ...
            if do_savefig_anim:
                plt.savefig('./ERA5/Figs/'+event+'.png', bbox_inches='tight', dpi=DPI, facecolor='w', edgecolor='k')
            else:
                figpath='./ERA5/Figs/'
